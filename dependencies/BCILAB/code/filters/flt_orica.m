...
arg_define([0 1],varargin,...
    ...
    arg_subswitch({'adaptiveFF','AdaptiveFF'},'cooling',{ ...
        ...
        'adaptive', { ...
            arg({'decayRateAlpha','DecayRate'},0.02,[0 Inf],sprintf('lambda = lambda - DecayRate*lambda + UpperBound*Gain*lambda^2')), ...
            arg({'upperBoundBeta','UpperBound'},0.001,[0 Inf],sprintf('lambda = lambda - DecayRate*lambda + UpperBound*Gain*lambda^2')), ...
            arg({'transBandWidthGamma','TransBandWidth'},1,[0 Inf],sprintf('Gain(z) ~ tanh((NSI/NSI_{min} - TransBandCenter) / TransBandWidth)')), ...
            arg({'transBandCenter','TransBandCenter'},5,[1 Inf],sprintf('Gain(z) ~ tanh((NSI/NSI_{min} - TransBandCenter) / TransBandWidth)')), ...
    ...
    arg_subtoggle({'evalConvergence','EvalConvergence'},[],...
        {...
            arg({'leakyAvgDelta','LeakyAvgNSI'},0.01,[0 1],sprintf('Leaky average value for computing NSI')), ...
            arg({'leakyAvgDeltaVar','LeakyAvgVar'},1e-3,[0 1],sprintf('Leaky average value for computing variance of source activation')), ...
